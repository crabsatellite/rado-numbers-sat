/-
  RadoNumbers/General/K3B4MultiplesCounting.lean
   ...
   — Diagnostic + analytic-bundle deliverable for
  ...
  **Diagnostic outcome ( Phase 1, `src/R440_b4_balanced_search.py`,
...
/-! ### Phase 4 — Candidate I' statement (def : Prop, NOT asserted). -/
...
/--
  ...
  ** Phase 1 SAT-verification status**: TRUE at $b = 4$. In fact,
...
/-! ### Phase 4 — analytic bundle for $b = 4$ at $n = 63$.

  We collect the kernel-pure analytic constraints that 's structural
  ...
  * Cross-position triples involving the v_4=2 multiples: $(16, 4, 8)$,
    $(32, 8, 16)$, $(48, 12, 24)$ etc., recorded as F2 in Phase 1.
  ...
  it is the maximum analytic content extracts at $b = 4$
  kernel-pure, parallel to 's `rounds_1_7_constraint_bundle`.
-/

/--
  ** b=4 self-loop bundle around the v_4 = 2 multiples.**
...
/--
  ** b=4 self-loop bundle, in numeric form.**
...
/-! ### Phase 4 — F2 multiple-of-multiple constraints at $b = 4$.
...
/--
  ** b=4 — pair-level forced-disagreements bundle.**

  A clean summary of the 6 forced-disagree inequalities at $b = 4$
  that 's structural probe identifies as the local "spine". Each
...
/--
  ** Stop C — formal record of the non-proof.**
  ...
  This `theorem` documents 's non-progress: any direct kernel-pure
