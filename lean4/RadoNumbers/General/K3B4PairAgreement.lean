/-
  RadoNumbers/General/K3B4PairAgreement.lean
   ...
   — Pair-agreement diagnostic at $b = 4$, $n = 32$
  for $\chi(8) = \chi(12)$.

  **Target statement ( attack).** For every valid mono-free
  ...
  data in `src/R440_chain_extraction.py`). 's prediction was that
  $n_{\text{eff}} = 32$ is exactly the smallest domain forcing
  $\chi(8) = \chi(12)$; Phase A confirms.

  ** Phase A (`src/R441_b4_n32_pair_agreement_search.py`).**
  ...
  ** Phase A — MUS analysis (`src/R441_b4_n32_mus_minimize.py`,
  ...
  ** Phase A — cross-checks.**
    ...
    * Pairs INVOLVING position 4 (i.e., $\chi(4) = \chi(\text{anything})$)
      are NOT forced at $n = 32$ — matches 's chain-extraction.
    * The v_4=2 pair $\chi(16) = \chi(32)$ is forced at $n = 32$.

  ** Phase A — conclusion: Stop C.** The 76-triple MUS, even at
  ...
  pattern identified at $n = 63$, just restricted.

  ** Phase B (this file).** We deliver:

  1. The statement `Chi8EqChi12K3B4_at32` as a `def : Prop` (parallel
     to 's `MultiplesSubColoringPopulationB4`). NOT asserted.
...
/-! ### Phase B — Pair-agreement statement (def : Prop, NOT asserted). -/

/--
  ** target — Pair agreement $\chi(8) = \chi(12)$ at $b = 4$,
  $n = 32$.**
  ...
  ** Phase A SAT-verification status**: TRUE. SAT diagnostic at
  ...
  ** Phase A analytic status**: STOP C. Best MUS over
  ...
  Stated as `def : Prop`; not asserted. 's analogous statement
  is `MultiplesSubColoringPopulationB4` (at $n = 63$). 
  refines: this PAIR-AGREEMENT sub-problem at $n = 32$ has the same
  Stop C verdict, with a slightly smaller MUS than 's 105-triple
  5-subset case.
-/
...
/-! ### Phase B — analytic bundle for $b = 4$ at $n = 32$.
  ...
  $n = 32$ (in contrast to 's $n = 60$ bundle). We use $m = 6$
...
/--
  ** b=4, n=32 self-loop bundle.**
  ...
  The first three are also in 's $n \ge 60$ bundle (at
...
/-! ### Phase B — single-pair extraction theorems.

  For paper readout, we expose the four inequalities individually.
-/

/--
  ** b=4, n=32 single inequality $\chi(16) \ne \chi(12)$.**
...
/--
  ** b=4, n=32 single inequality $\chi(20) \ne \chi(16)$.**
...
/--
  ** b=4, n=32 single inequality $\chi(32) \ne \chi(24)$.**
...
/-! ### Phase B — F2 mono-block at $b = 4$, $n = 32$.
...
/-! ### Phase B — Stop C documentation.

  We DO NOT prove `Chi8EqChi12K3B4_at32`. Phase A MUS extraction
  shows:
  ...
  ** Phase A — what would be needed for a kernel-pure proof:**
  ...
  This is the EXACT case-explosion pattern / identified;
  shrinking the domain from $n = 63$ to $n = 32$ does NOT compactify
  the MUS by an order of magnitude.

  **Strategic finding ()**: pair-agreement at the safe-zone
  ...
  ** recommendation**: do NOT continue chasing forward pair-
  ...
  inherits the same Stop C blocker. Pivot to either:
    ...
    (iii) The BACKWARD direction: instead of proving $\chi(8)
        ...
        not part of scope; flagged for.
-/

/--
  ** Stop C — formal record of the non-proof.**
