/-
  ...
  R438 (2026-05-21) — analytic attack on `CascadeCompressionHyp b 3`
...
/-! ### R438 Phase 2 — minimal $k = 3$ compression lemma statement. -/

/--
  **R438 Sublemma 1 statement (target, OPEN for general $b \ge 3$).**
...
/--
  **R438 — definitional equivalence.**

  `MultiplesOmitColorK3 b` (the R438 sublemma statement) is
...
/-! ### R438 Phase 4 — Rounds 1-7 constraint bundle (PARTIAL).
...
/--
  **R438 Phase 4 — Rounds 1-7 unconditional constraint bundle.**
...
/-! ### R438 Phase 4 — explicit blocker theorem statement.
...
/--
  **R438 Phase 4 — explicit blocker (Stop C diagnosis).**
  ...
  **Status**: OPEN for general $b \ge 4$.  Closed for $b = 3$ via
  the kernel-pure R246 derivation (`K3B3.lean:79-80`).
...
/-! ### R438 — note on the iff with the main goal.
  ...
  **The honest report**: R438 STOPS at this point with Stop C —
  the structural obstruction (depth-2 case analysis on
  multiples-of-$b^2$) is exactly the boundary that separates
  $b = 3$ (closeable kernel-pure via R246's partition-shape
