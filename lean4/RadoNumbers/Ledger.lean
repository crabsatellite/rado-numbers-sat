...
def gap_r5_296_sat : GapEntry := {
  ...
  paperSource :=
    ...
    "0.2s/step"
  attackHistory := [
    "v0.1: Cat 2 SAT-verified atomic; no explicit witness encoded " ++
      "(the SAT solver returns a satisfying assignment for each " ++
      "$n$ but the paper does not transcribe these)",
    "v0.1 Phase 0+4: 16-pattern audit CLEAN — paper Theorem " ++
      "`thm:r5` second half + §'Breakdown of the b^k Pattern' " ++
      "matches statement (incremental SAT, 244 ≤ n ≤ 296, all SAT, " ++
      "0.2s/step); not folkloric-inflation (#9, scope is exactly " ++
      "$R_5(3) > 296$); paper's narrative noting 12h+ timeout at " ++
      "$n = 297$ supports likely sharp transition (not part of " ++
      "this atomic claim — separate conjecture)"
  ]
