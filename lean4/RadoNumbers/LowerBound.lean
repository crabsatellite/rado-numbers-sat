...
/-- For `b ≥ 2` and `0 < n < b^k`, the b-adic valuation satisfies
    ...
    ANALYTICALLY PROVEN Round 16 (2026-05-15) — previously a Cat 2
...
/-- Ultrametric, strict-comparison form: if `v_b(d) < v_b(y)`, then
    ...
    ANALYTICALLY PROVEN Round 17 (2026-05-15) — previously a Cat 2
...
/--
  **Round 43 Theorem (valuation's multiples omit zero).**
  ...
  **Significance**: a concrete realization of compression by the
  ...
  vacuity-equivalent `CascadeCompressionHyp` (Round 42).
-/
...
/-! ### Round 44 (2026-05-15) — the valuation realizes the
...
/--
  **Round 44 Theorem (valuation's omitted-color distance pair).**
...
/--
  **Round 44 Corollary (valuation's distance pair monochromatic
  at 0).**
...
/--
  **Round 47 Theorem (b-unit lifts to color 0 at distance $b^{k-1}$).**
...
/-! ### Round 48 (2026-05-15) — valuation of $b^c \cdot u$ and
  ...
  * **`bAdicVal_distance_pair_color_c`** — for any interior color
  ...
  $c = k - 1$ needs a separate argument (Round 49). -/

/--
  **Round 48 Lemma — $v_b(b^c \cdot u) = c$ for b-units.**
...
/--
  **Round 48 Theorem (interior-color distance pair under valuation).**
...
/-! ### Round 49 (2026-05-15) — boundary-color distance pair
    under the valuation.
...
/--
  **Round 49 Lemma — $v_b(2) = 0$ for $b \ge 3$.**
...
/--
  **Round 49 Theorem (boundary-color distance pair).**
...
/-! ### Round 50 (2026-05-15) — the valuation realizes
    `DistancePairProperty` body at every color.

  Rounds 47–49 supplied distance-$b^{k-1}$ pairs at color 0
  (Round 47), interior colors $c \in [1, k-2]$ (Round 48), and
  the boundary color $c = k - 1$ (Round 49) under the valuation
...
/--
  **Round 50 Theorem (valuation realizes DPP body at every color).**
...
/-! ### Round 51 (2026-05-15) — extract valuation's mono-freeness
...
/--
  **Round 51 Lemma — `bAdicVal_avoidsMono`.**
...
/--
  **Round 51 Lemma — `bAdicVal_isValidColoring`.**
...
/--
  **Round 52 Theorem (b-adic unique factorization).**
...
/--
  **Round 53 Theorem (iff characterization of valuation
  stratification).**
...
/--
  **Round 51 Theorem (DPP body is realizable).**
  ...
  Composes `bAdicVal_isValidColoring` (Round 51),
  `bAdicVal_avoidsMono` (Round 51), and
  `bAdicVal_distance_pair_witness` (Round 50).
